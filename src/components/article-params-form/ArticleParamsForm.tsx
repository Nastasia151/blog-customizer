...
export const ArticleParamsForm = ({ mainRef }: ArticleParamsFormProps) => {
	...
	const [draftState, setDraftState] = useState<ArticleStateType>(defaultArticleState);
  ...
  function getCurrentOption(options: OptionType[], cssVar: string): OptionType {
    ...
    const value = mainRef.current.style.getPropertyValue(cssVar) ||
...
function setCssVars(state: ArticleStateType) {
    ...
    mainRef.current.style.setProperty('--font-family', state.fontFamilyOption.value);
    mainRef.current.style.setProperty('--font-size', state.fontSizeOption.value);
    mainRef.current.style.setProperty('--font-color', state.fontColor.value);
    mainRef.current.style.setProperty('--container-width', state.contentWidth.value);
    mainRef.current.style.setProperty('--bg-color', state.backgroundColor.value);
  }
   ...
   return (
    <>
      ...
      {isOpen && (
        <aside 
			...
			ref={rootRef}>
          <form onSubmit={handleApply} onReset={handleReset}>
            <RadioGroup
              name="fontFamily"
	...
	// useOutsideClickClose({
 	// 	isOpen,
  	// 	rootRef,
  	// 	onClose,
  	// 	onChange: onClose, // закрыть при клике вне
	// });
...
// 			{isOpen && (
// 			<aside ref={rootRef} className={styles.container}>
